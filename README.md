...
- **Codex web search** — automatic web-search tool injection for Codex-routed conversations; intercepts search tool calls from streamed responses, executes them against the Codex search API, and feeds results back transparently (configurable timeout, max iterations, base URL, and chat output so results survive context compression)

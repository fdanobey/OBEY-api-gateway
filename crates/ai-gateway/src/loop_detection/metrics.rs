use dashmap::DashMap;
use std::{collections::VecDeque, sync::{atomic::{AtomicU64, Ordering}, Mutex}, time::Instant};
...
impl ConfidenceHistogram {
    ...
    fn observe(&self, value: f32) {
        ...
        self.sum_micros
            .fetch_add((value as f64 * 1_000_000.0).round() as u64, Ordering::Relaxed);
        if let Some(index) = CONFIDENCE_BUCKETS.iter().position(|boundary| value <= *boundary) {
...
impl LoopDetectionMetrics {
    ...
    pub fn record_eviction(&self) {
        ...
        let now = Instant::now();
        let mut times = self.eviction_times.lock().expect("eviction metrics mutex poisoned");
        times.push_back(now);
        while times.front().is_some_and(|timestamp| now.duration_since(*timestamp).as_secs() > 300) {
            times.pop_front();
        }
    }
    ...
    pub fn write_prometheus(&self, out: &mut String, active_sessions: usize) {
        ...
        let mut labels = self.confidence.iter().map(|entry| entry.key().clone()).collect::<Vec<_>>();
        labels.sort();
        for label in labels {
            let Some(histogram) = self.confidence.get(&label) else { continue };
            let mut cumulative = 0;
            for (index, boundary) in CONFIDENCE_BUCKETS.iter().enumerate() {
                cumulative += histogram.buckets[index].load(Ordering::Relaxed);
                out.push_str(&format!("obey_loop_confidence_score_bucket{{virtual_key=\"{}\",le=\"{}\"}} {}\n", escape_label(&label), boundary, cumulative));
            }
            let count = histogram.count.load(Ordering::Relaxed);
            out.push_str(&format!("obey_loop_confidence_score_bucket{{virtual_key=\"{}\",le=\"+Inf\"}} {}\n", escape_label(&label), count));
            out.push_str(&format!("obey_loop_confidence_score_sum{{virtual_key=\"{}\"}} {}\n", escape_label(&label), histogram.sum_micros.load(Ordering::Relaxed) as f64 / 1_000_000.0));
            out.push_str(&format!("obey_loop_confidence_score_count{{virtual_key=\"{}\"}} {}\n", escape_label(&label), count));
        }
        ...
        let mut enforcement = self.enforcement.iter().map(|entry| (entry.key().clone(), entry.value().load(Ordering::Relaxed))).collect::<Vec<_>>();
        enforcement.sort_by(|left, right| left.0.cmp(&right.0));
        for ((level, virtual_key), count) in enforcement {
            out.push_str(&format!("obey_loop_enforcement_total{{level=\"{}\",virtual_key=\"{}\"}} {}\n", escape_label(&level), escape_label(&virtual_key), count));
        }
        ...
        out.push_str(&format!("obey_loop_sessions_evicted_total {}\n", self.evicted_total()));
    }
}
...
fn private_virtual_key_label(virtual_key: Option<&str>) -> String {
    virtual_key.map(|value| format!("id:{value}")).unwrap_or_else(|| "none".to_string())
}

...
impl GatewayExtractionAdapter {
    ...
    fn parse_response(content: &str) -> Result<Vec<StructuredMemoryCandidate>, MemoryExtractionProviderError> {
    ...
    let raw_candidates: Vec<RawCandidate> = serde_json::from_str(json_str).map_err(|e| {
...
impl MemoryExtractionProvider for GatewayExtractionAdapter {
    ...
    if !status.is_success() {
        ...
        message: format!("extraction provider returned HTTP {}: {}", status.as_u16(), error_text),
        });
    }

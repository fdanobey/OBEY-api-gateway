...
use axum::{body::{to_bytes, Body}, http::{Request, Response, StatusCode}, Extension};
use serde_json::Value;
use std::{convert::Infallible, sync::{Arc, Mutex}, time::{Duration, Instant}};
...
fn detector(loop_config: LoopDetectionConfig) -> (Arc<LoopDetectorState>, LoopDetectorLayer) {
    ...
    let state = Arc::new(LoopDetectorState::new(Arc::new(RwLock::new(config)), loop_config));
...
async fn hard_stop_short_circuits_inner_service() {
    ...
    let service = layer.layer(service_fn(move |_request: Request<Body>| { let calls = calls_inner.clone(); async move { calls.fetch_add(1, std::sync::atomic::Ordering::Relaxed); Ok::<_, Infallible>(Response::new(Body::from("{}"))) } }));
    for _ in 0..5 { service.clone().oneshot(request("hard-stop")).await.unwrap(); }
    ...
    assert_eq!(state.sessions.get("hard-stop").unwrap().enforcement_level, EnforcementLevel::HardStop);
}
...
async fn throttle_applies_minimum_delay_and_warning_header() {
    ...
    let service = layer.layer(service_fn(|_request: Request<Body>| async { Ok::<_, Infallible>(Response::builder().header("content-type","application/json").body(Body::from("{}")).unwrap()) }));
    for _ in 0..3 { service.clone().oneshot(request("throttle")).await.unwrap(); }
...
async fn injection_mutation_is_observed_by_inner_service() {
    ...
    let service = layer.layer(service_fn(move |request: Request<Body>| { let observed=observed_inner.clone(); async move { let body=to_bytes(request.into_body(),1024*1024).await.unwrap(); observed.lock().unwrap().push(serde_json::from_slice(&body).unwrap()); Ok::<_,Infallible>(Response::builder().header("content-type","application/json").body(Body::from("{}")).unwrap()) } }));
    for _ in 0..5 { service.clone().oneshot(request("inject")).await.unwrap(); }
    let observed = observed.lock().unwrap();
    let messages = &observed[4]["messages"];
    assert!(messages.as_array().unwrap().iter().any(|message| message["role"] == "system"));
}

#[tokio::test]
async fn disabled_mode_creates_no_state_and_reload_clears_state() {
    let (state, layer) = detector(LoopDetectionConfig::default());
    let service = layer.layer(service_fn(|_request: Request<Body>| async { Ok::<_,Infallible>(Response::new(Body::empty())) }));
    service.oneshot(request("disabled")).await.unwrap();
    assert!(state.sessions.is_empty());

    let server = GatewayServer::new(config(LoopDetectionConfig::default()), None).await.unwrap();
    server.state.loop_detector.sessions.insert("old".into(), SessionState::new(None, 5));
...
async fn event_delivery_reaches_live_subscriber() {
    ...
    let event = tokio::time::timeout(Duration::from_millis(500), subscription.receiver.recv()).await.unwrap().unwrap();
    assert_eq!(event.session_id, "event");
}

fn authenticated_key() -> AuthenticatedKey {
    AuthenticatedKey { id:"vk-ordering".into(), name:None, status:KeyStatus::Active, budget_limit_usd:None, token_budget:None, budget_window:None, current_spend_usd:0.0, current_tokens_used:0, window_start:None, requests_per_minute:None, tokens_per_minute:None, model_access:None, expires_at:None, loop_detection:None }
}
...
async fn actual_gateway_router_places_loop_after_auth_and_before_guardrail() {
    ...
    for _ in 0..4 { let _ = app.clone().oneshot(request("router-order")).await; }
    let response = app.oneshot(request("router-order")).await.unwrap();
    assert_eq!(response.status(), StatusCode::FORBIDDEN, "guardrail must observe loop-injected prompt");
    assert_eq!(server.state.loop_detector.sessions.get("router-order").unwrap().vk_id.as_deref(), Some("vk-ordering"));
}
